...
public class CreateListingController implements Initializable, LifecycleAwareController {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        NetworkService.getInstance().getClient()
            .addResponseHandler(EventType.CREATE_AUCTION, HANDLER_ID, this::handleCreateAuctionResponse);
    }
    ...
    private void handleSubmit() {
        ...
        if (title == null || title.isBlank() || category == null || description == null || description.isBlank()) {
        ...
        try {
            startDateTime = LocalDateTime.of(startDateInput.getValue(), LocalTime.parse(startTimeInput.getText()));
            endDateTime = LocalDateTime.of(endDateInput.getValue(), LocalTime.parse(endTimeInput.getText()));
        ...
        if (selectedImageFile == null && (uploadedImageUrl == null || uploadedImageUrl.isBlank())) {
            if (imageUrlInput.getText() != null && !imageUrlInput.getText().isBlank()) {
                uploadedImageUrl = imageUrlInput.getText(); // Chấp nhận nếu gõ URL thủ công thay vì chọn File
            } else {
        ...
        if (selectedImageFile != null && uploadedImageUrl == null) {
            // Xin chữ ký từ mây
            com.auction.core.dto.item.GetUploadSignatureRequest req = new com.auction.core.dto.item.GetUploadSignatureRequest("auction_items");
            
            // Xóa handler lỡ có trước đó
            NetworkService.getInstance().getClient().removeResponseHandler(EventType.GET_UPLOAD_SIGNATURE, HANDLER_ID);
            NetworkService.getInstance().getClient().addResponseHandler(EventType.GET_UPLOAD_SIGNATURE, HANDLER_ID, rs -> {
                Map<?, ?> map = null;
                try {
                    map = (Map<?, ?>) JsonMapper.fromJson((String) rs, Map.class);
                } catch (Exception e) {}
                
                if (map != null && map.containsKey("status") && "success".equals(map.get("status"))) {
                    ...
                    uploadToCloudinaryAsync(selectedImageFile, signature, timestamp, apiKey, "auction_items",
                        (secureUrl) -> {
                            this.uploadedImageUrl = secureUrl; // cache
                            sendCreateAuctionRequest(sellerId, title, description, category, secureUrl, startPrice, increment, startDateTime, endDateTime);
                        },
                        (err) -> {
                            Platform.runLater(() -> showAlert("Upload Error", "Could not upload image: " + err));
                        });
                } else {
                    Platform.runLater(() -> showAlert("Error", "Could not acquire upload signature: " + JsonMapper.toJson(rs)));
                }
            });
            NetworkService.getInstance().sendRequest(EventType.GET_UPLOAD_SIGNATURE, req);
        } else {
            sendCreateAuctionRequest(sellerId, title, description, category, uploadedImageUrl, startPrice, increment, startDateTime, endDateTime);
        }
    }
    
    private void sendCreateAuctionRequest(Integer sellerId, String title, String description, String category, String imageUrl, Double startPrice, Double increment, LocalDateTime startDateTime, LocalDateTime endDateTime) {
        CreateAuctionRequest request = new CreateAuctionRequest(
            sellerId, title, description, category, imageUrl,
            startPrice, increment, startDateTime, endDateTime
        );
        NetworkService.getInstance().sendRequest(EventType.CREATE_AUCTION, request);
    }

    private void uploadToCloudinaryAsync(java.io.File file, String signature, long timestamp, String apiKey, String folder,
        ...
        new Thread(() -> {
            try {
                ...
                java.util.function.BiConsumer<String, String> addFormField = (name, value) -> {
                ...
                if (response.statusCode() == 200 || response.statusCode() == 201) {
                    @SuppressWarnings("unchecked")
                    Map<String, Object> map = (Map<String, Object>) JsonMapper.fromJson(response.body(), Map.class);
                    String secureUrl = (String) map.get("secure_url");
                    if (secureUrl != null) onSuccess.accept(secureUrl);
                    else onError.accept("Secure URL missing in response");
                } else {
