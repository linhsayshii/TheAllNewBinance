 ...
 *
 * Receives {@code {sellerId: <int>}} via DataReceivable when the user clicks
 * a seller's name on the Auction Detail page (or Sellers page).
 *
 * Shows only the seller's public Live and Sold listings.
 * No Dashboard, Finance, or My Bids sections.
 */
public class PublicSellerController implements DataReceivable, LifecycleAwareController {
    ...
    @FXML private void handleNavListings()    { /* single nav item — always visible */ }
    @FXML private void handleListingsTabLive() { renderTab(viewModel.getLiveListings()); }
    @FXML private void handleListingsTabSold() { renderTab(viewModel.getSoldListings()); }
    ...
    private void renderTab(List<ProfileAuctionCardUiModel> cards) {
        ...
        if (!empty) cards.forEach(c -> listingsCardsContainer.getChildren().add(loadCard(c)));
    }
    ...
    @Override
    public void onUnload() { /* nothing to clean up */ }
}
