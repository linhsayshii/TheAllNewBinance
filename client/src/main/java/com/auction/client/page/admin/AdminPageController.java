 ...
 *
 * Two sections:
 *  1. User Management — TableView listing all users (ID, username, email, role, balance, status)
 *  2. Listing Management — FlowPane of all auctions (Active/Pending/Sold/Unsold tabs),
 *     each card has a [ Force Promote ] button (admin can set isFeatured without payment).
 *
 * Security: All admin API calls are double-checked on the server by role.
 *           Client-side visibility is for UX only.
 */
public class AdminPageController implements LifecycleAwareController {
    ...
    private void initialize() {
        ...
        navGroup.selectedToggleProperty().addListener((obs, old, sel) -> {
            if (sel == null) { old.setSelected(true); return; }
            showSection(sel);
        });
    ...
    private void showSection(Toggle sel) {
        usersSection  .setVisible(sel == navUsers);    usersSection  .setManaged(sel == navUsers);
        listingsSection.setVisible(sel == navListings); listingsSection.setManaged(sel == navListings);

        if (!dataLoaded) {
            dataLoaded = true;
            loadDataAsync();
        } else {
            if (sel == navListings) renderCurrentListingTab();
        }
    }
                            ...
                            for (Object entry : list) {
                                ...
                                if (u != null) {
                                    userRows.add(new UserRow(
                                        ...
                                        "$" + MONEY_FMT.format(u.getBalance() != null ? u.getBalance() : 0.0),
                                        "Active"
                                    ));
                                }
                            }
                ...
                try {
                    ...
                    if (Boolean.TRUE.equals(resp.get("success"))) {
                        ...
                        if (data instanceof List<?> list) {
                            for (Object entry : list) {
                                String json = JsonMapper.toJson(entry);
                                PublicAuctionDto dto = JsonMapper.fromJson(json, PublicAuctionDto.class);
                                if (dto != null) allAuctions.add(dto);
                            }
                        }
                    }
                } finally {
    ...
    @FXML private void handleTabActive()  { renderListings("ACTIVE"); }
    @FXML private void handleTabPending() { renderListings("PENDING"); }
    @FXML private void handleTabSold()    { renderListings("ENDED"); }
    @FXML private void handleTabUnsold()  { renderListings("CANCELLED"); }

    private void renderCurrentListingTab() {
        Toggle sel = listingTabGroup.getSelectedToggle();
        if (sel == tabPending)  { renderListings("PENDING"); return; }
        if (sel == tabSold)     { renderListings("ENDED"); return; }
        if (sel == tabUnsold)   { renderListings("CANCELLED"); return; }
    ...
    private void renderListings(String statusFilter) {
        ...
        List<PublicAuctionDto> filtered = allAuctions.stream()
        ...
        if (!empty) {
            for (PublicAuctionDto dto : filtered) {
                ...
                if (card != null) listingsContainer.getChildren().add(card);
            }
        }
    }
    ...
    private void forcePromote(PublicAuctionDto dto) {
        ...
        NetworkService.getInstance().addCorrelationHandler(corr, raw ->
            Platform.runLater(() -> {
                ...
                String msg = ok
                    ? "✓ Promoted: " + (dto.getItemName() != null ? dto.getItemName() : "Auction #" + dto.getAuctionId())
                    : "✗ Failed: " + resp.get("message");
                Alert alert = new Alert(ok ? Alert.AlertType.INFORMATION : Alert.AlertType.ERROR, msg);
    ...
    @FXML private void handleNavUsers()    { /* handled by listener */ }
    @FXML private void handleNavListings() { /* handled by listener */ }
    ...
    @Override
    public void onUnload() { /* no cleanup needed */ }

    // ── Inner data class ──────────────────────────────────────────────────

    public record UserRow(Integer id, String username, String email, String role, String balance, String status) {}
}
