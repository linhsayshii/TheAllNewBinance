 ...
 *
 * This page is for the currently logged-in user only:
 *  — Dashboard: balance summary + active bid/listing counts
 *  — Finance:   wallet details, deposit/withdraw
 *  — My Bids:   active bids + won auctions
 *  — My Listings: live / pending / sold / unsold tabs
 *
 * Navigation here always starts from the header's "Profile" link,
 * which passes {@code {userId: <current user id>}} via DataReceivable.
 */
public class PersonalProfileController implements DataReceivable, LifecycleAwareController {

    private static final String PROFILE_CARD_FXML = "/fxml/components/item/profile-auction-card.fxml";
    ...
    private void showSection(Toggle selected) {
        List<Node> all = List.of(dashboardSection, financeSection, myBidsSection, myListingsSection);
        for (Node n : all) { n.setVisible(false); n.setManaged(false); }
        ...
        if (selected == navDashboard) {
            ...
            show(myBidsSection);
            if (!dataLoaded) loadDataAsync();
            else renderBidsTab(bidsTabActive.isSelected() ? viewModel.getActiveBids() : viewModel.getWonAuctions());
    ...
    private void show(Node n) { n.setVisible(true); n.setManaged(true); }
    ...
    private void bindDashboard() {
        ...
        viewModel.activeBidsCountProperty().addListener((obs, o, n) ->
            Platform.runLater(() -> dashActiveBidsLabel.setText(n.toString())));
        viewModel.activeListingsCountProperty().addListener((obs, o, n) ->
            Platform.runLater(() -> dashActiveListingsLabel.setText(n.toString())));
    }
    ...
    private void loadDataAsync() {
        ...
        java.util.concurrent.CompletableFuture.runAsync(() -> {
            ...
            Platform.runLater(() -> {
                dashActiveBidsLabel.setText(String.valueOf(viewModel.activeBidsCountProperty().get()));
                dashActiveListingsLabel.setText(String.valueOf(viewModel.activeListingsCountProperty().get()));

                Toggle current = navGroup.getSelectedToggle();
                if (current == navMyBids) {
                    renderBidsTab(bidsTabActive.isSelected() ? viewModel.getActiveBids() : viewModel.getWonAuctions());
    ...
    @FXML private void handleBidsTabActive() { renderBidsTab(viewModel.getActiveBids()); }
    @FXML private void handleBidsTabWon()    { renderBidsTab(viewModel.getWonAuctions()); }
    ...
    private void renderBidsTab(List<ProfileAuctionCardUiModel> cards) {
        ...
        if (!empty) cards.forEach(c -> bidsCardsContainer.getChildren().add(loadCard(c)));
    }
    ...
    @FXML private void handleListingsTabLive()    { renderListingsTab(viewModel.getLiveListings()); }
    @FXML private void handleListingsTabPending() { renderListingsTab(viewModel.getPendingListings()); }
    @FXML private void handleListingsTabSold()    { renderListingsTab(viewModel.getSoldListings()); }
    @FXML private void handleListingsTabUnsold()  { renderListingsTab(viewModel.getUnsoldListings()); }
    ...
    private void renderListingsTab(List<ProfileAuctionCardUiModel> cards) {
        ...
        if (!empty) {
            ...
            cards.forEach(c -> {
                Runnable onPromote = isLiveOrPending
    ...
    private void refreshListingsStats() {
        ...
        listingsStatsLabel.setText(total + " listings · " + sold + " sold · "
                + viewModel.getSellerSuccessRate() + " success rate");
    }
    ...
    private void openPromoteModal(Integer auctionId, String title) {
        try {
            ...
            modal.open(modalHost, root, auctionId, title, () -> {
    ...
    @FXML private void handleNavDashboard()  { /* handled by ToggleGroup listener */ }
    @FXML private void handleNavFinance()    { /* handled by ToggleGroup listener */ }
    @FXML private void handleNavMyBids()     { /* handled by ToggleGroup listener */ }
    @FXML private void handleNavMyListings() { /* handled by ToggleGroup listener */ }
    ...
    @FXML private void handleDeposit()  { showComingSoon("Deposit"); }
    @FXML private void handleWithdraw() { showComingSoon("Withdraw"); }

    private void showComingSoon(String feature) {
        new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.INFORMATION,
                feature + " functionality is coming soon!").show();
    }
    ...
    @Override
    public void onUnload() { /* no persistent resources to clean up */ }
}
