...
/**
 * Controller for admin-auction-card.fxml.
 * Similar to ProfileAuctionCardController but:
 *  — Receives a {@link PublicAuctionDto} (from the admin all-auctions endpoint)
 *  — Shows a [Force Promote] button when a Runnable callback is provided
 */
public class AdminAuctionCardController {
    ...
    private void handleMousePressed(MouseEvent e) {
        if (e != null && e.getButton() == MouseButton.PRIMARY) pressedAt = System.nanoTime();
    }
    ...
    private void handleOpenAuction(MouseEvent e) {
        ...
        if (auctionId != null) {
            NavigationService.getInstance().navigateTo(
                SceneRegistry.AUCTION_PAGE, Map.of("auctionId", auctionId));
        }
    }

    @FXML
    private void handleForcePromote() {
        if (onForcePromote != null) onForcePromote.run();
    }
    ...
    public void setData(PublicAuctionDto dto, Runnable onForcePromote) {
        ...
        titleLabel.setText(dto.getItemName() != null ? dto.getItemName() : "Auction #" + dto.getAuctionId());
        ...
        badgeLabel.getStyleClass().add(switch (status) {
    ...
    private String buildTimeLabel(PublicAuctionDto dto) {
        ...
        long h = rem.toHours();
        if (h >= 24) return String.format("Ends in %dd %dh", h / 24, h % 24);
    ...
    private boolean isPrimaryClick(MouseEvent e) {
        ...
        return e != null && e.getButton() == MouseButton.PRIMARY
            && e.isStillSincePress() && ms <= MAX_CLICK_MS;
    }
}
