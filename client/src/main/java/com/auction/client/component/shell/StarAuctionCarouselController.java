 ...
 *
 * <p>Flow: 1. initialize() → gửi GET_FEATURED_AUCTIONS tới mock/server 2. Nhận response → build
 * list StarAuctionCard và thêm vào carouselTrack 3. Mỗi card chiếm đúng 100% width của viewport
 * (prefWidth = viewport.width) 4. Slide bằng TranslateTransition trên carouselTrack 5. Dot
 * indicators cập nhật theo slide hiện tại
 */
public class StarAuctionCarouselController {
        ...
        try {
            NetworkService.getInstance()
                    ...
                    .addResponseHandler(
                            EventType.PLACE_BID,
                            "CAROUSEL_PLACE_BID",
                            this::handleCarouselPlaceBid);
        } catch (Exception e) {
            System.err.println(
                    "[StarCarousel] Network client not ready, real-time pricing disabled: "
                            + e.getMessage());
        }
    }
    ...
    private void handleCarouselPlaceBid(String rawJson) {
        try {
            ...
            com.auction.core.auction.Bid incomingBid =
                    JsonMapper.fromJson(
                            JsonMapper.toJson(data), com.auction.core.auction.Bid.class);
                ...
                Platform.runLater(
                        () -> {
                            for (StarAuctionCardController ctrl : controllers) {
                                if (ctrl.getAuctionId() != null
                                        && ctrl.getAuctionId() == bidAuctionId) {
    ...
    private void buildCarousel() {
        ...
        // 2. Support Seamless Infinite Loop: append a clone of the first card at the end of the
        // track
        if (items.size() > 1) {
    ...
    /**
     * Slides the carousel track to reveal card at {@code targetIndex}. Supports seamless
     * cloning-based infinite looping.
     */
    private void slideTo(int targetIndex) {
        ...
        tt.setOnFinished(
                ev -> {
                    ...
                    // If we scrolled forward to the clone card at index n, instantly snap back to
                    // real card at 0
                    if (currentIndex == n) {
                        carouselTrack.setTranslateX(0);
                        currentIndex = 0;
