...
/**
 * Template for controller methods — eliminates repeated
 * null-check → deserialize → call service → wrap response → catch boilerplate.
 */
public abstract class BaseController {

    /**
     * Handles a synchronous request with the standard controller flow:
     * 1. Null-check payload
     * 2. Deserialize JSON → DTO
     * 3. Execute handler (business logic)
     * 4. Wrap result in success/error response
     *
     ...
     */
    protected <T, R> String handleSync(String request, Class<T> requestType,
                                       Function<T, R> handler, String fallbackError) {
        if (request == null) {
    ...
    protected <T> CompletableFuture<String> handleAsync(
            String request, Class<T> requestType,
            Function<T, CompletableFuture<Object>> handler, String fallbackError) {
        if (request == null) {
