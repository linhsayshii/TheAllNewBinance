...
public class AuctionController extends BaseController {
    ...
    public String getAuctionDetails(String payload) {
        try {
            GetAuctionDetailsRequest getAuctionDetailsRequest =
                    JsonMapper.fromJson(payload, GetAuctionDetailsRequest.class);
            com.auction.core.dto.auction.AuctionDetailsDto details =
                    auctionService
                            .getAuctionDetails(getAuctionDetailsRequest.getAuctionId())
                            .join();
            if (details == null) {
    ...
    public String getAuctionsBySellerId(String payload) {
        try {
            GetAuctionBySellerIdRequest getAuctionBySellerIdRequest =
                    JsonMapper.fromJson(payload, GetAuctionBySellerIdRequest.class);
            List<Auction> auctions =
                    auctionService.getAuctionsBySellerId(getAuctionBySellerIdRequest).join();
            if (auctions == null) {
    ...
    public String getPublicAuctions(String payload) {
        try {
            ...
            List<com.auction.core.dto.auction.PublicAuctionDto> auctions =
                    auctionService.getPublicAuctions(request).join();
            return JsonMapper.toJson(successResponse(auctions));
        } catch (IllegalArgumentException ex) {
            return JsonMapper.toJson(errorResponse(ex.getMessage()));
        } catch (Exception ex) {
    ...
    public String promoteAuction(String payload) {
        try {
            PromoteAuctionRequest request =
                    JsonMapper.fromJson(payload, PromoteAuctionRequest.class);
            Boolean success = auctionService.promoteAuction(request).join();
            if (Boolean.TRUE.equals(success)) {
                return JsonMapper.toJson(successResponse(null, "Promoted successfully!"));
            }
            return JsonMapper.toJson(errorResponse("Promote failed, please try again."));
    ...
    public String getFeaturedAuctions(String payload) {
        try {
            GetFeaturedAuctionsRequest request =
                    (payload == null || payload.isBlank() || payload.equals("null"))
